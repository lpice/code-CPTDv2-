...
site <- read_excel('Sites.xlsx') 
# read the HWSD soil ID raster
soil_ID <- raster('hwsd_raster.tif') 
...
# load the HWSD soil properties database
...
soil <- soil_info %>% select(c(4,1:3,5:60))
# save the extracted results
write.csv(soil,file='soil.csv')